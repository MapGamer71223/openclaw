# Starts the full stack (backend + frontend) for local development on Windows.
# Usage: .\run.ps1
...
# Forward EVERY variable from .env into the new backend window, not just
# DATA_DIR/DEMO_MODE -- otherwise AI_DETECTOR_CONFIGURED, BRAVE_API_KEY,
# OPENCLAW_ENABLED, etc. silently never reach uvicorn's process and Python
# falls back to config.py's hardcoded defaults instead of your .env.
...
Write-Host "== Frontend: installing npm deps ==" -ForegroundColor Cyan
Set-Location "$RootDir\frontend"
if (-not (Test-Path "node_modules")) {
    npm install
}

Write-Host "== Frontend: starting Vite dev server on :5173 ==" -ForegroundColor Cyan
npm run dev
