...
def get_source_provider(
    ...
    if settings.GOOGLE_VISION_API_KEY and (image_path or image_url):
        base = GoogleVisionSourceProvider(settings.GOOGLE_VISION_API_KEY, image_path=image_path, image_url=image_url)
        return _PlaceholderThumbnailFillingProvider(_WaybackEnrichingProvider(base))
    if settings.PICIMAGESEARCH_ENABLED and (image_path or image_url):
        engines = [e.strip() for e in settings.PICIMAGESEARCH_ENGINES.split(",") if e.strip()]
        base = PicImageSearchProvider(image_path=image_path, image_url=image_url, engines=engines)
        return _PlaceholderThumbnailFillingProvider(_WaybackEnrichingProvider(base))
    if settings.MRISA_URL and image_url:
        return _PlaceholderThumbnailFillingProvider(_WaybackEnrichingProvider(MrisaSourceProvider(settings.MRISA_URL, image_url)))
    if settings.BRAVE_API_KEY:
        return _PlaceholderThumbnailFillingProvider(BraveSourceProvider(settings.BRAVE_API_KEY))
    return _PlaceholderThumbnailFillingProvider(DDGSourceProvider())
