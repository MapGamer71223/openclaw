...
def embed_batch(records: List[dict], storage_dir: Path) -> None:
    ...
    Mutates `records` (via slice-assignment) to drop any record whose
    file couldn't be embedded -- e.g. it existed at write time but was
    gone by read time (seen with OneDrive-synced folders / antivirus
    scanning on Windows). Those images are skipped rather than crashing
    the whole batch or inserting a row with no embedding.
    """
    ...
    for rec, vec in zip(records, vectors):
        ...
        kept.append(rec)
    if n_dropped:
        logger.warning("Dropped %d/%d images this batch (embedding failed -- see embeddings.py warnings above)",
