...
def embed_images_batch(paths: List[Path], batch_size: int = 16) -> List[Optional[List[float]]]:
    ...
    Resilient to individual unreadable files (deleted/quarantined/moved
    between write and read -- seen in practice on Windows with
    OneDrive-synced project folders or antivirus real-time scanning of
    freshly-downloaded images): a file that fails to open gets a `None`
    in its output slot instead of crashing the whole batch. Callers must
    check for `None` and drop that record rather than insert a fake/zero
    embedding.
    """
    _load_model()
    import torch
    ...
    for i in range(0, len(paths), batch_size):
        ...
        batch_vectors: List[Optional[List[float]]] = [None] * len(batch_paths)
        if images:
            ...
            for out_idx, vec in zip(ok_indices, vecs):
                batch_vectors[out_idx] = vec
